-- Forum System Database Schema (Fixed Version)
-- Integrates with existing CTF roles system for moderation
-- Fixed to work with actual CTF roles schema (role_name instead of role_id)
...
CREATE TABLE IF NOT EXISTS forum_categories (
    ...
    requires_role TEXT, -- Optional CTF role requirement to view/post
...
-- Check if CTF roles system exists before creating role-based policies
DO $$ 
BEGIN
    ...
    IF EXISTS (
        ...
        -- Create admin policies that work with the actual CTF roles schema
        CREATE POLICY "Only admins can manage categories" ON forum_categories
            FOR ALL USING (auth.uid() IN (
                SELECT ucr.user_id FROM user_ctf_roles ucr
                WHERE ucr.role_name IN ('admin', 'ctf_admin')
                AND ucr.is_active = true
            ));
    ELSE
        -- Fallback to simple admin check if CTF roles don't exist
        CREATE POLICY "Only admins can manage categories" ON forum_categories
            FOR ALL USING (auth.uid() IN (
...
-- Check if CTF roles system exists for thread moderation policies
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        ...
        -- Create moderation policies using actual CTF roles schema
        CREATE POLICY "Authors and moderators can update threads" ON forum_threads
            FOR UPDATE USING (
    ...
    ELSE
        -- Fallback to author-only editing if CTF roles don't exist
...
-- Check if CTF roles system exists for post moderation policies
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        ...
        -- Create moderation policies using actual CTF roles schema
        CREATE POLICY "Authors and moderators can update posts" ON forum_posts
            FOR UPDATE USING (
                auth.uid() = author_id OR 
                auth.uid() IN (
                    SELECT ucr.user_id FROM user_ctf_roles ucr
...
BEGIN
    IF EXISTS (
        ...
        -- Create moderation log policies using CTF roles
        CREATE POLICY "Moderators can view moderation log" ON forum_moderation_log
        ...
        CREATE POLICY "Moderators can insert moderation log" ON forum_moderation_log
...
-- Update CTF roles to include forum moderation permissions (if CTF system exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'ctf_roles') THEN
        UPDATE ctf_roles 
        SET permissions = permissions || '{"manage_forum_moderation": true}'
        WHERE name IN ('admin', 'ctf_admin', 'ctf_head_referee');
    END IF;
END $$; 