...
import { Download, ChevronDown, ChevronRight, ExternalLink, Monitor, FileText, Tag } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchReleases = async () => {
      try {
        ...
        // Filter out asset-only releases
        const appReleases = data.filter(r => r.tag_name !== 'assets-v1');
        setReleases(appReleases);
        // Auto-expand the latest
        if (appReleases.length > 0) {
          setExpandedRelease(appReleases[0].tag_name);
        }
      } catch (err: any) {
        ...
        <section className="mb-12">
          ...
          {error && (
            <div className="p-4 rounded-lg border border-red-500/30 bg-red-500/10 text-red-300 text-sm">
              Failed to load releases from GitHub. <a href={`https://github.com/${GITHUB_REPO}/releases`} target="_blank" rel="noopener noreferrer" className="underline">View on GitHub</a>
            </div>
          )}
