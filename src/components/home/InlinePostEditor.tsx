      ...
      return (
        <div key={key} className="my-4">
          <img
            src={node.attrs?.src}
            alt={node.attrs?.alt || ''}
            className="max-w-full rounded-lg border border-cyan-500/20"
          />
        </div>
      );
    case 'listItem':
  ...
  if (node.type === 'image') {
    return (
      <img
        ...
        src={node.attrs?.src}
        alt={node.attrs?.alt || ''}
        className="max-w-full rounded-lg border border-cyan-500/20 my-2 inline-block"
      />
    );
  }
  return null;
    ...
    <div className="fixed inset-0 z-50 bg-gray-950/95 backdrop-blur-sm flex flex-col">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center rounded-lg border border-gray-700 overflow-hidden mr-2">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium transition-colors ${viewMode === 'editor' ? 'bg-cyan-600 text-white' : 'text-gray-400 hover:bg-gray-800'}`}
            >
              <Edit3 className="w-3.5 h-3.5 inline mr-1" />Editor
            </button>
            <button
              onClick={() => setViewMode('split')}
              className={`px-3 py-1.5 text-xs font-medium transition-colors ${viewMode === 'split' ? 'bg-cyan-600 text-white' : 'text-gray-400 hover:bg-gray-800'}`}
            >
              Split
            </button>
            <button
              onClick={() => setViewMode('preview')}
              className={`px-3 py-1.5 text-xs font-medium transition-colors ${viewMode === 'preview' ? 'bg-cyan-600 text-white' : 'text-gray-400 hover:bg-gray-800'}`}
            >
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex items-center gap-1.5 px-4 py-1.5 bg-green-600 hover:bg-green-500 disabled:bg-green-800 text-white text-sm font-medium rounded-lg transition-colors"
          >
      ...
      <div className="px-4 py-3 border-b border-gray-800 bg-gray-900/60 space-y-2">
        <input
          ...
          className="w-full bg-transparent text-xl font-bold text-white placeholder-gray-600 focus:outline-none border-b border-transparent focus:border-cyan-500/40 pb-1"
        ...
        <div className="flex gap-4">
          <input
            ...
            className="flex-1 bg-transparent text-sm text-gray-300 placeholder-gray-600 focus:outline-none border-b border-transparent focus:border-cyan-500/30 pb-1"
          ...
          <input
            ...
            className="w-64 bg-transparent text-sm text-cyan-300 placeholder-gray-600 focus:outline-none border-b border-transparent focus:border-cyan-500/30 pb-1 font-mono"
            placeholder="Tags: update, news..."
          />
        </div>
      </div>
      ...
      {viewMode !== 'preview' && (
        <div className="px-4 py-2 border-b border-gray-800 bg-gray-900/40 flex flex-wrap items-center gap-0.5">
          ...
          <span className="ml-auto text-[10px] text-gray-600 font-mono">Paste images directly into the editor</span>
        </div>
      )}
        ...
        {viewMode !== 'editor' && (
          <div className={`${viewMode === 'split' ? 'w-1/2' : 'w-full'} overflow-y-auto bg-gray-950/80`}>
            <div className="p-2">
              ...
              {/* Preview rendered like the actual post */}
              <div className="relative rounded-lg border border-cyan-500/30 bg-gray-950/90 overflow-hidden">
                ...
                <div className="relative z-20 p-6">
                  ...
                  {subtitle && (
                    <p className="text-cyan-100/50 text-base mb-5 font-medium italic">{subtitle}</p>
                  )}
                  {/* Content frame */}
