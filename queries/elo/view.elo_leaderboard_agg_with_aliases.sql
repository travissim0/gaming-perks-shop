CREATE OR REPLACE VIEW elo_leaderboard_agg_with_aliases AS
    WITH normalized AS (
      SELECT
    ...
    -- Primary profiles from profile_aliases (add DISTINCT to prevent duplicates)
    profile_mapping AS (
      ...
      ORDER BY LOWER(alias), is_primary DESC, added_at ASC
    ),
    -- Profiles without aliases (add DISTINCT)
    profile_direct_mapping AS (
      ...
      WHERE LOWER(in_game_alias) NOT IN (
        SELECT DISTINCT LOWER(alias) FROM profile_aliases
      )
    ),
    -- Combine profile sources (change to UNION to eliminate duplicates)
    all_profile_mapping AS (
      SELECT * FROM profile_mapping
      UNION  -- Changed from UNION ALL to UNION
      SELECT * FROM profile_direct_mapping
    ),
    -- Aliases aggregated by profile (fix duplicates and prioritize complete alias lists)
    profile_aliases_aggregated AS (
      ...
      FROM (
        SELECT
          profile_id,
          STRING_AGG(DISTINCT alias, ', ' ORDER BY alias) as all_aliases,
          COUNT(alias) as alias_count
        FROM profile_aliases
        GROUP BY profile_id
        UNION
        SELECT
      ...
      ORDER BY profile_id, alias_count DESC  -- Prioritize the entry with more aliases
    ),
    -- Create grouping keys (add DISTINCT to prevent duplicates)
    grouped_names AS (
      SELECT DISTINCT
        n.player_name_normalized,
        n.game_mode,
        COALESCE(apm.primary_normalized_name, n.player_name_normalized) as
  grouping_key,
        apm.profile_id,
        apm.primary_alias
      FROM normalized n
    ...
    aggregated_individual AS (
      SELECT
        ...
        ROUND((
          CASE
            WHEN SUM(total_games) > 0
            THEN SUM(elo_confidence::float * total_games) / SUM(total_games)       
            ELSE 0
          END
        ...
        ROUND((
          CASE
            WHEN SUM(total_games) > 0
            THEN SUM(weighted_elo::float * total_games) / SUM(total_games)
            ELSE 0
          END
        ...
        ROUND((
          CASE
            WHEN SUM(total_games) > 0
            THEN SUM(kill_death_ratio::float * total_games) / SUM(total_games)     
            ELSE 0
          END
  ...
  n.player_name_normalized
      ...
      GROUP BY gn.grouping_key, n.game_mode, gn.profile_id, gn.primary_alias       
    ),
  ...
  n.player_name_normalized
    ...
    display_names AS (
      SELECT DISTINCT ON (grouping_key, game_mode)
        aa.grouping_key,
        aa.game_mode,
        COALESCE(aa.primary_alias, ln.latest_cased_name) as display_name
      FROM all_aggregated aa
      LEFT JOIN latest_names ln ON ln.player_name_normalized = aa.grouping_key     
        AND (ln.game_mode = aa.game_mode OR aa.game_mode = 'Combined')
      ORDER BY grouping_key, game_mode, aa.primary_alias IS NOT NULL DESC
    )
    SELECT DISTINCT  -- Add final DISTINCT to ensure no duplicates in result       
    ...
    LEFT JOIN profile_aliases_aggregated paa ON paa.profile_id = a.profile_id      
