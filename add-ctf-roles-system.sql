...
CREATE TABLE IF NOT EXISTS ctf_roles (
...
CREATE TABLE IF NOT EXISTS user_ctf_roles (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Add video and commentator fields to matches table
ALTER TABLE matches 
ADD COLUMN IF NOT EXISTS video_url TEXT,
ADD COLUMN IF NOT EXISTS video_platform TEXT CHECK (video_platform IN ('youtube', 'twitch', 'vimeo', 'other')),
ADD COLUMN IF NOT EXISTS video_title TEXT,
ADD COLUMN IF NOT EXISTS recorded_by UUID REFERENCES profiles(id),
ADD COLUMN IF NOT EXISTS commentary_enabled BOOLEAN DEFAULT FALSE;
...
CREATE INDEX IF NOT EXISTS idx_user_ctf_roles_user_id ON user_ctf_roles(user_id);
CREATE INDEX IF NOT EXISTS idx_user_ctf_roles_role_name ON user_ctf_roles(role_name);
CREATE INDEX IF NOT EXISTS idx_referee_applications_applicant ON referee_applications(applicant_id);
CREATE INDEX IF NOT EXISTS idx_referee_applications_status ON referee_applications(status);
CREATE INDEX IF NOT EXISTS idx_match_commentators_match ON match_commentators(match_id);
CREATE INDEX IF NOT EXISTS idx_match_commentators_commentator ON match_commentators(commentator_id);
