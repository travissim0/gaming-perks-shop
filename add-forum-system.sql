...
CREATE POLICY "Only admins can manage categories" ON forum_categories
    FOR ALL USING (auth.uid() IN (
        ...
        JOIN ctf_roles cr ON ucr.role_id = cr.id
        WHERE cr.name IN ('site_admin', 'ctf_admin')
    ));
...
-- Update CTF roles to include forum moderation permissions
UPDATE ctf_roles 
SET permissions = permissions || '{"manage_forum_moderation": true}'
WHERE name IN ('site_admin', 'ctf_admin', 'ctf_head_referee'); 