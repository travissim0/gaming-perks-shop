-- Freeinf dueling tournament: additive first installation, run as postgres.
-- Creates 7 tables, 15 functions (9 server RPCs), indexes and 2 triggers.
-- RLS has no policies. All RPCs are server-only, including reads.
-- Every statement is transactional. Existing installations/collisions abort.
-- The profiles mapping and director UUID were supplied by the repository owner.
begin;
do $$
begin
  ...
  if not exists(select 1 from pg_attribute where attrelid=to_regclass('public.profiles') and attname='id' and atttypid='uuid'::regtype and not attisdropped) then
    raise exception 'Expected public.profiles.id of type uuid; no changes applied.';
  end if;
  if exists(select 1 from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname ~ '^dueling_tournament_')
    or exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname ~ '^dueling_tournament_') then
    raise exception 'Tournament objects already exist. Stop and inspect; this is a first-install migration.';
  end if;
  if (select count(*) from pg_roles where rolname in ('anon','authenticated','service_role')) <> 3 then
    raise exception 'Expected Supabase anon, authenticated and service_role roles.';
  end if;
end $$;

create table public.dueling_tournament_records (
  id text primary key,
...
create table public.dueling_tournament_directors (
  user_id uuid primary key references public.profiles(id),
  granted_at timestamptz not null default now()
);
create table public.dueling_tournament_operations (
...
create table public.dueling_tournament_rate_buckets (
  bucket_key text primary key,
  window_start timestamptz not null,
  hits integer not null
);
create table public.dueling_tournament_registration_cooldowns (
  event_id text not null references public.dueling_tournament_records(id),
  actor_id text not null,
...
create table public.dueling_tournament_history (
...
create index dueling_tournament_history_page_idx on public.dueling_tournament_history(event_id,kind,sequence desc);
create table public.dueling_tournament_notice_reads (
  event_id text not null references public.dueling_tournament_records(id) on delete cascade,
...
create function public.dueling_tournament_grant_lock()
...
create trigger dueling_tournament_director_lock before insert or update or delete on public.dueling_tournament_directors
for each row execute function public.dueling_tournament_grant_lock();

create index dueling_tournament_rate_window_idx on public.dueling_tournament_rate_buckets(window_start);

-- SECURITY DEFINER: Updates private admission counters without granting the runtime direct table writes.
create function public.dueling_tournament_rate_check(p_key text, p_limit integer)
...
-- SECURITY DEFINER: Reads private director grants and referee assignments without exposing either table.
create function public.dueling_tournament_capabilities(p_actor_id text)
returns jsonb language sql stable security definer set search_path = pg_catalog, public as $$
  select jsonb_build_object(
    'director', exists(select 1 from public.dueling_tournament_directors where user_id::text = p_actor_id),
    'referee', exists(select 1 from public.dueling_tournament_records where state->'refereeIds' ? p_actor_id)
  );
$$;

-- SECURITY DEFINER: Reads the private aggregate and notice acknowledgements after actor visibility checks.
create function public.dueling_tournament_get(p_locator text, p_actor_id text)
returns jsonb language sql stable security definer set search_path = pg_catalog, public as $$
  ...
  where (id = p_locator or slug = p_locator)
    and ((state->>'published')::boolean or state->'refereeIds' ? p_actor_id
      or exists(select 1 from public.dueling_tournament_directors where user_id::text = p_actor_id))
  order by (id = p_locator) desc limit 1;
$$;

-- SECURITY DEFINER: Lists only actor-visible aggregates from private storage.
create function public.dueling_tournament_list(p_actor_id text, p_offset integer default 0)
returns jsonb language sql stable security definer set search_path = pg_catalog, public as $$
...
create function public.dueling_tournament_history_guard()
returns trigger language plpgsql set search_path=pg_catalog,public as $$
begin
...
create trigger dueling_tournament_history_guard_trigger before update or delete on public.dueling_tournament_history
for each row execute function public.dueling_tournament_history_guard();

create function public.dueling_tournament_validate_delta(p_old jsonb,p_new jsonb,p_command text,p_actor text)
...
begin
  v_allowed := case p_command
    ...
    when 'cancel' then array['phase','paused','pauseReason','announcements','published','featured']
    else null end;
  if v_allowed is null then raise exception using errcode='PT422',message='Unknown tournament command.'; end if;
  if p_command='cancel' and (p_new->'published' is distinct from 'false'::jsonb or p_new->'featured' is distinct from 'false'::jsonb) then
    raise exception using errcode='PT422',message='Cancelled events must be removed from public discovery.';
  end if;
...
create function public.dueling_tournament_compact(p_state jsonb)
...
-- SECURITY DEFINER: Writes a notice acknowledgement only after checking its recipient and admission budget.
create function public.dueling_tournament_ack(p_event_id text,p_actor_id text,p_notice_id text)
...
-- SECURITY DEFINER: Reads private operation receipts scoped to the verified actor.
create function public.dueling_tournament_receipt(p_actor_id text,p_operation_id uuid)
returns jsonb language sql stable security definer set search_path = pg_catalog, public as $$
...
-- SECURITY DEFINER: Reads archived history after event, staff and recipient checks.
create function public.dueling_tournament_history_page(p_event_id text,p_actor_id text,p_kind text,p_before bigint default null)
...
begin
  ...
  if p_kind not in ('audit','draws','notices','announcements') or (p_kind='audit' and not exists(select 1 from public.dueling_tournament_directors where user_id::text = p_actor_id)) or (p_kind='notices' and p_actor_id is null) then
...
create function public.dueling_tournament_validate(p_state jsonb)
returns void language plpgsql set search_path = pg_catalog, public as $$
...
create function public.dueling_tournament_source(p_state jsonb, p_source jsonb, p_depth integer default 0)
...
-- SECURITY DEFINER: Atomically creates a draft and receipt after checking private director grants.
create function public.dueling_tournament_create(p_actor_id text, p_operation_id uuid, p_fingerprint text, p_state jsonb)
...
begin
  ...
  if not exists(select 1 from public.dueling_tournament_directors where user_id::text = p_actor_id) then
...
-- SECURITY DEFINER: Atomically validates and saves private event, history, notices and receipts under locks.
create function public.dueling_tournament_commit(p_actor_id text, p_event_id text, p_expected_revision bigint, p_operation_id uuid, p_fingerprint text, p_command_type text, p_state jsonb)
returns jsonb language plpgsql security definer set search_path = pg_catalog, public as $$
declare
  v_compacted jsonb;
...
begin
  ...
  select exists(select 1 from public.dueling_tournament_directors where user_id::text = p_actor_id) into v_director;
...
-- Revoke inherited Supabase default grants, including service_role defaults.
-- All entry RPCs are SECURITY DEFINER, so the server needs no direct table or
-- sequence privileges. Internal invoker functions execute as the RPC owner.
alter table public.dueling_tournament_records enable row level security;
revoke all on table public.dueling_tournament_records from public, anon, authenticated, service_role;
alter table public.dueling_tournament_directors enable row level security;
revoke all on table public.dueling_tournament_directors from public, anon, authenticated, service_role;
alter table public.dueling_tournament_operations enable row level security;
revoke all on table public.dueling_tournament_operations from public, anon, authenticated, service_role;
alter table public.dueling_tournament_rate_buckets enable row level security;
revoke all on table public.dueling_tournament_rate_buckets from public, anon, authenticated, service_role;
alter table public.dueling_tournament_registration_cooldowns enable row level security;
revoke all on table public.dueling_tournament_registration_cooldowns from public, anon, authenticated, service_role;
alter table public.dueling_tournament_history enable row level security;
revoke all on table public.dueling_tournament_history from public, anon, authenticated, service_role;
alter table public.dueling_tournament_notice_reads enable row level security;
revoke all on table public.dueling_tournament_notice_reads from public, anon, authenticated, service_role;
revoke all on sequence public.dueling_tournament_history_sequence_seq from public, anon, authenticated, service_role;
revoke all on function public.dueling_tournament_grant_lock() from public, anon, authenticated, service_role;
revoke all on function public.dueling_tournament_rate_check(text,integer) from public, anon, authenticated, service_role;
grant execute on function public.dueling_tournament_rate_check(text,integer) to service_role;
revoke all on function public.dueling_tournament_capabilities(text) from public, anon, authenticated, service_role;
grant execute on function public.dueling_tournament_capabilities(text) to service_role;
revoke all on function public.dueling_tournament_get(text,text) from public, anon, authenticated, service_role;
grant execute on function public.dueling_tournament_get(text,text) to service_role;
revoke all on function public.dueling_tournament_list(text,integer) from public, anon, authenticated, service_role;
grant execute on function public.dueling_tournament_list(text,integer) to service_role;
revoke all on function public.dueling_tournament_history_guard() from public, anon, authenticated, service_role;
revoke all on function public.dueling_tournament_validate_delta(jsonb,jsonb,text,text) from public, anon, authenticated, service_role;
revoke all on function public.dueling_tournament_compact(jsonb) from public, anon, authenticated, service_role;
revoke all on function public.dueling_tournament_ack(text,text,text) from public, anon, authenticated, service_role;
grant execute on function public.dueling_tournament_ack(text,text,text) to service_role;
revoke all on function public.dueling_tournament_receipt(text,uuid) from public, anon, authenticated, service_role;
grant execute on function public.dueling_tournament_receipt(text,uuid) to service_role;
revoke all on function public.dueling_tournament_history_page(text,text,text,bigint) from public, anon, authenticated, service_role;
grant execute on function public.dueling_tournament_history_page(text,text,text,bigint) to service_role;
revoke all on function public.dueling_tournament_validate(jsonb) from public, anon, authenticated, service_role;
revoke all on function public.dueling_tournament_source(jsonb,jsonb,integer) from public, anon, authenticated, service_role;
revoke all on function public.dueling_tournament_create(text,uuid,text,jsonb) from public, anon, authenticated, service_role;
grant execute on function public.dueling_tournament_create(text,uuid,text,jsonb) to service_role;
revoke all on function public.dueling_tournament_commit(text,text,bigint,uuid,text,text,jsonb) from public, anon, authenticated, service_role;
grant execute on function public.dueling_tournament_commit(text,text,bigint,uuid,text,text,jsonb) to service_role;

-- Foreign key makes a missing or mistyped owner-supplied profile roll back everything.
insert into public.dueling_tournament_directors(user_id)
values ('66efeaff-8a9e-4ef3-95d1-acad7f6d402b'::uuid);

-- Verify installation before commit; any failure rolls back the whole installation.
do $$
declare
  v_tables integer;
  ...
  v_service_mismatch integer;
begin
  select count(*), count(*) filter (where c.relrowsecurity),
    coalesce(sum((select count(*) from pg_policy p where p.polrelid=c.oid)),0)
  into v_tables, v_rls_tables, v_policies
  from pg_class c join pg_namespace n on n.oid=c.relnamespace
  where n.nspname='public' and c.relkind='r' and c.relname ~ '^dueling_tournament_';
  if v_tables <> 7 or v_rls_tables <> 7 or v_policies <> 0 then
    raise exception 'Tournament verification failed: expected seven RLS tables and zero policies.';
  end if;
  ...
  select count(*),
    ...
    count(*) filter (where has_function_privilege('service_role',p.oid,'EXECUTE') is distinct from p.prosecdef)
  into v_functions, v_anon_execute, v_authenticated_execute, v_service_execute, v_service_mismatch
  from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  where n.nspname='public' and p.proname ~ '^dueling_tournament_';
  if v_functions <> 15 then
    raise exception 'Tournament verification failed: expected fifteen functions.';
  end if;
  if v_anon_execute <> 0 or v_authenticated_execute <> 0 then
    raise exception 'Tournament verification failed: anonymous or authenticated function access remains.';
  end if;
  if v_service_execute <> 9 or v_service_mismatch <> 0 then
    raise exception 'Tournament verification failed: expected exactly nine server RPC grants.';
  end if;
  if not exists (
    select 1 from public.dueling_tournament_directors
    where user_id='66efeaff-8a9e-4ef3-95d1-acad7f6d402b'::uuid
  ) then
    raise exception 'Tournament verification failed: the director grant is missing.';
  end if;
end $$;
commit;

-- Keep the single summary row last so SQL editors display it after COMMIT.
select 'migration_verified' as status, t.*, f.*,
  exists(select 1 from public.dueling_tournament_directors
    where user_id='66efeaff-8a9e-4ef3-95d1-acad7f6d402b'::uuid) as director_granted
from (
  select count(*)::integer as tables_verified,
    (count(*) filter (where c.relrowsecurity))::integer as rls_tables_verified,
    coalesce(sum((select count(*) from pg_policy p where p.polrelid=c.oid)),0)::integer as policy_count
  from pg_class c join pg_namespace n on n.oid=c.relnamespace
  where n.nspname='public' and c.relkind='r' and c.relname ~ '^dueling_tournament_'
) t cross join (
  select count(*)::integer as functions_verified,
    (count(*) filter (where has_function_privilege('anon',p.oid,'EXECUTE')))::integer as anon_executable_functions,
    (count(*) filter (where has_function_privilege('authenticated',p.oid,'EXECUTE')))::integer as authenticated_executable_functions,
    (count(*) filter (where has_function_privilege('service_role',p.oid,'EXECUTE')))::integer as service_executable_functions
  from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  where n.nspname='public' and p.proname ~ '^dueling_tournament_'
) f;
