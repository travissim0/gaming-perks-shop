-- Add Media Manager Role and Match Video Management
...
CREATE POLICY "CTF roles can manage match videos" ON matches
    FOR UPDATE USING (
        auth.uid() IN (
            ...
            AND cr.permissions ? 'add_match_videos'
            AND cr.permissions->>'add_match_videos' = 'true'
        )
...
BEGIN
    ...
    IF NEW.youtube_url IS NOT NULL AND NEW.youtube_url != OLD.youtube_url AND NEW.video_thumbnail_url IS NULL THEN
